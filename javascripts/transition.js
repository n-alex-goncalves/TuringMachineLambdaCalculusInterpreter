//  a single element over an arbitrary list
function enumerate_function(list, index) {	
	if (index < 0) {
		throw 'ENUMERATE FUNCTION ERROR: Index is not a positive!' 
	}
	let term = '';
	let l = list.length;
	for (let i = 1; i < l + 1; i++) {
...
// finite strings over an arbitrary alphabet
...
function step_configuration_function(tape_alphabet, states, ruleset) {
	...
	l = states.length
	let N, M, cur_state, new_state, new_char, new_dir;
	for (let i = 0; i < l; i++) {
		M = 'λu.λa.λv.a';
		cur_state = states[i];
		for (let j = 0; j < tape_alphabet.length; j++) {
			if (cur_state === 'SH' || (cur_state in ruleset) == false) {
				N = 'λu.λv.λx.xu' + enumerate_function(tape_alphabet, j) + 'v' + enumerate_function(states, i);
			} 
			else {
				let [new_state, new_char, new_dir] = ruleset[states[i]][tape_alphabet[j]];
				if (new_dir === 'L') {
